...
def generate_neural_param_combinations(param_dict):
    """
    Genera todas las combinaciones posibles de parámetros para redes neuronales
    """
...
def format_neural_param_value(value):
    """
    Formatea los valores de parámetros de redes neuronales para mostrar en el DataFrame
    """
    if value is None:
        return None
...
def create_neural_results_dataframe(variables_data, file_order=0, source_file=None):
    """
    Crea un DataFrame con todas las combinaciones y resultados de redes neuronales
    ...
    for var_data in variables_data:
        ...
        combinations, _ = generate_neural_param_combinations(param_dict)
        ...
        for i in range(max_items):
            ...
            for param_name, param_value in combo.items():
                row[param_name] = format_neural_param_value(param_value)
...
def find_neural_files(directory_path, pattern="*.txt"):
    """
    Encuentra todos los archivos de redes neuronales que coinciden con el patrón en el directorio
    ...
    # Filtrar archivos que contengan información de redes neuronales
    # Puedes ajustar este filtro según tus necesidades
    neural_files = [f for f in files if any(keyword in os.path.basename(f).lower() 
                                          for keyword in ['nn', 'neural', 'mlp', 'net'])]
    
    # Si no se encuentran archivos con palabras clave específicas, devolver todos
    if not neural_files:
        neural_files = files
    
    return neural_files

def debug_neural_file_structure(file_path):
    """
    Función de diagnóstico para examinar la estructura del archivo de redes neuronales
    """
    print(f"\n=== DIAGNÓSTICO DE ARCHIVO NEURAL: {os.path.basename(file_path)} ===")
    ...
    # Buscar patrones específicos de redes neuronales
    pattern = r'(\w+)\s*\n\[\s*\{\s*([^}]+)\s*\}\s*\]\s*\n\[([-\d\.\s]+)\]\s*\n\[([-\d\.\s]+)\]\s*\n\[([\d\s]+)\]'
    matches = re.findall(pattern, content, re.DOTALL)
...
def process_multiple_neural_files(directory_path, output_path=None, file_pattern="*.txt", debug=False):
    """
    Procesa múltiples archivos de redes neuronales y los combina en un solo CSV
    MANTIENE EL ORDEN ORIGINAL DE LOS ARCHIVOS Y DATOS
    """
    try:
        # Encontrar todos los archivos de redes neuronales en el directorio
        print(f"Buscando archivos de redes neuronales en: {directory_path}")
        neural_files = find_neural_files(directory_path, file_pattern)
        
        if not neural_files:
            print("No se encontraron archivos de redes neuronales en el directorio")
            return None
        
        print(f"Encontrados {len(neural_files)} archivos:")
        for i, file in enumerate(neural_files):
            print(f"  {i+1}. {os.path.basename(file)}")
        
        # Diagnóstico opcional
        if debug:
            for file_path in neural_files[:1]:  # Solo el primer archivo para diagnóstico
                debug_neural_file_structure(file_path)
        
        # Procesar cada archivo MANTENIENDO EL ORDEN
        all_dataframes = []
        
        for file_order, file_path in enumerate(neural_files):
            print(f"\nProcesando archivo {file_order + 1}/{len(neural_files)}: {os.path.basename(file_path)}")
            ...
            try:
                ...
                df = create_neural_results_dataframe(variables_data, file_order=file_order, source_file=file_path)
        ...
        if output_path is None:
            output_path = "neural_network_results_grouped_by_variable.csv"
...
def analyze_neural_parameters(combined_df):
    """
    Analiza específicamente los parámetros de las redes neuronales
    """
    print("\n=== ANÁLISIS DE PARÁMETROS DE REDES NEURONALES ===")
    ...
    if 'regressor__lr' in combined_df.columns:
        print("\nAnálisis por learning rate:")
        lr_analysis = combined_df.groupby('regressor__lr').agg({
            'score': ['count', 'mean', 'std'],
            'variable': 'nunique'
        }).round(4)
        print(lr_analysis)
    ...
    # Analizar epochs
    if 'regressor__epochs' in combined_df.columns:
        print("\nAnálisis por epochs:")
        epochs_analysis = combined_df.groupby('regressor__epochs').agg({
            'score': ['count', 'mean', 'std'],
            'variable': 'nunique'
        }).round(4)
...
if __name__ == "__main__":
    ...
    directory_path = "../../../sk_learn/rna/guardados"
    
    # Procesar todos los archivos con diagnóstico habilitado
    combined_df = process_multiple_neural_files(directory_path, debug=True)
    ...
    if combined_df is not None:
        ...
        # Análisis específico de parámetros de redes neuronales
        analyze_neural_parameters(combined_df)
