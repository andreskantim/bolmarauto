...
def parse_param_dict_svm(param_str):
    ...
    try:
        ...
        # Manejar nombres de parámetros con diferentes formatos
        clean_str = re.sub(r"'regressor__(\w+)'", r"'regressor\1'", clean_str)
        clean_str = re.sub(r"'regressor(\w+)'", r"'regressor\1'", clean_str)
...
def extract_regressor_from_filename(filename):
    """
    Extrae el tipo de regresor del nombre del archivo - CORREGIDO
    """
    filename = filename.lower()
    
    # Buscar patrones más específicos
    if filename.startswith('linsvr') or 'linsvr' in filename:
        return 'LinSVR'
    elif filename.startswith('sdg') or filename.startswith('sgd') or 'sdg' in filename or 'sgd' in filename:
        return 'SDG'
    elif filename.startswith('svr') or 'svr' in filename:
        return 'SVR'
    else:
        # Fallback más específico
        print(f"Advertencia: No se pudo determinar el tipo de regresor para {filename}, usando SVR por defecto")
        return 'SVR'
...
def parse_svm_results_file(file_path):
    ...
    print(f"Procesando archivo: {os.path.basename(file_path)} -> Regresor: {regressor_type}")
...
def normalize_svm_parameters(param_dict, regressor_type):
    ...
    CORREGIDO: Eliminadas columnas polydegree y nystroem_n_components
    CORREGIDO: Mejorado el manejo de regressor_loss
    """
    normalized_params = {}
    ...
    base_params = {
        ...
        'regressor_degree': param_dict.get('regressor__degree', [3]),  # Por defecto 3
        'pca': param_dict.get('pca', [None]),
        'regressor_epsilon': param_dict.get('regressor__epsilon', [None]),
        'regressor_kernel': param_dict.get('regressor__kernel', ['poly']),  # Por defecto poly
    ...
    elif 'regressorC' in param_dict:
        base_params['regressor_C'] = param_dict['regressorC']
    elif 'regressoralpha' in param_dict:
        base_params['regressor_C'] = param_dict['regressoralpha']
    
    # Manejar regressor_degree con diferentes nombres
    if 'regressor__degree' in param_dict:
        base_params['regressor_degree'] = param_dict['regressor__degree']
    elif 'regressordegree' in param_dict:
        base_params['regressor_degree'] = param_dict['regressordegree']
    
    # Manejar kernel con diferentes nombres
    if 'regressor__kernel' in param_dict:
        base_params['regressor_kernel'] = param_dict['regressor__kernel']
    elif 'regressorkernel' in param_dict:
        base_params['regressor_kernel'] = param_dict['regressorkernel']
    
    # Manejar epsilon con diferentes nombres
    if 'regressor__epsilon' in param_dict:
        base_params['regressor_epsilon'] = param_dict['regressor__epsilon']
    elif 'regressorepsilon' in param_dict:
        base_params['regressor_epsilon'] = param_dict['regressorepsilon']
    
    # Manejar loss con diferentes nombres - CORREGIDO
    if 'regressor__loss' in param_dict:
        base_params['regressor_loss'] = param_dict['regressor__loss']
    elif 'regressorloss' in param_dict:
        base_params['regressor_loss'] = param_dict['regressorloss']
    elif 'regressor_loss' in param_dict:
        base_params['regressor_loss'] = param_dict['regressor_loss']
...
def process_multiple_svm_files(directory_path, output_path=None, file_pattern="*.txt", debug=False):
    ...
    try:
        ...
        for file_order, file_path in enumerate(svm_files):
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    # Mostrar el tipo de regresor detectado
                    print(f"  - Tipo de regresor detectado: {df['regressor'].iloc[0]}")
                else:
        ...
        # Verificar tipos de regresores únicos
        print(f"Tipos de regresores encontrados: {combined_df['regressor'].unique()}")
        ...
        # Reorganizar columnas con regressor_degree en segunda posición
        columns = list(df_for_export.columns)
        if 'regressor_degree' in columns:
            columns.remove('regressor_degree')
            # Insertar regressor_degree después de regressor_C
            c_index = columns.index('regressor_C') if 'regressor_C' in columns else 2
            columns.insert(c_index + 1, 'regressor_degree')
            df_for_export = df_for_export[columns]
        ...
        print("✓ Se ha incluido regressor_degree en segunda posición (por defecto 3)")
        print("✓ Se han eliminado las columnas polydegree y nystroem_n_components")
        print("✓ Se ha corregido el manejo de regressor_loss")
...
def analyze_svm_parameters(combined_df):
    ...
    # Analizar degree
    if 'regressor_degree' in combined_df.columns:
        print(f"\nAnálisis por degree:")
        degree_analysis = combined_df[combined_df['regressor_degree'] != 'None'].groupby(['regressor_degree', 'regressor']).agg({
            'score': ['count', 'mean', 'std']
        }).round(4)
        print(degree_analysis)
    
    # Analizar loss - CORREGIDO
...
if __name__ == "__main__":
    ...
    if combined_df is not None:
        ...
        print(best_results[['variable', 'regressor', 'score', 'regressor_C', 'regressor_degree', 'regressor_epsilon']])
    else:
